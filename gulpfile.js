...
const isTest = ( process.argv[3] || '' ).toUpperCase() === '--TEST';
            ...
            closureCompiler(
                {
                    ...
                    define            : [
                        ...
                        'DEFINE_REGEXP_COMPAT__NODEJS=' + isTest,
                    ...
                    compilation_level : strCompileType === 'develop' && clientMinEsVersion !== 2 && !isTest ? 'SIMPLE' : 'ADVANCED',
                    ...
                    output_wrapper    : '// ReRE.js for ' +
                        (
                            isTest ? 'Node' : 'ES' + clientMinEsVersion
                        )
                        + '[' + strCompileType + '], Compat feature:ES' + ecmaFeatureVersion + ', (https://githug.com/itozyun/rerejs)\n' +
                        (
                            isTest ? '%output%' :
                    ...
                    js_output_file    : isTest ? 'ReRE.Node.js' :
                                        clientMinEsVersion !== 2 ?
                                            'ReRE.es' + clientMinEsVersion + '.' + ecmaFeatureVersion + '.' + strCompileType + '.js' :
                                            'ReRE.es2.js'
                }
            )
        ).pipe( gulp.dest( clientMinEsVersion !== 2 && !isTest ? 'dist/' + strCompileType : tempDir ) );
    },
    function( cb ){
        if( clientMinEsVersion !== 2 || isTest ){
            return cb();
        };
        return gulp.src(
            [
