...
const charSetDigit = new CharSet();
charSetDigit.add(0x30, 0x39 + 1); // 0..9

/** A `CharSet` which does not contain ASCII digits. */
const charSetInvertDigit = charSetDigit.clone().invert();
...
const charSetInvertWord = charSetWord.clone().invert();
...
const charSetInvertUnicodeWord = charSetUnicodeWord.clone().invert();
...
const charSetSpace = new CharSet(category.get('Space_Separator'));
...
const charSetInvertSpace = charSetSpace.clone().invert();
